...
pub async fn walk<P: AsRef<std::path::Path>>(path_ref: P, root: fstream::SendRoot) -> Result<()> {
	...
	Ok(if let Some(dir) = root.dir(path.clone()).await.context(ErrFstream)? {
...
#[async_recursion]
pub async fn walk_dir(path: &mut std::path::PathBuf, dir: fstream::SendDir) -> Result<Option<fstream::SendDir>> {
	let mut dir = dir;
...
pub async fn walk_file(path: &mut std::path::PathBuf, file: fstream::SendFile) -> Result<fstream::SendDir> {
	let mut file = file;
	...
	loop {
		...
		match file.data(data).await.context(ErrFstream)? {
			...
			fstream::SendFileAction::Skip(dir) => {
				return Ok(dir)
			},
		}
	}
